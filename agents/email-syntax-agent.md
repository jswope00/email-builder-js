...
- **Configurable in the editor** (typical): `title`, `numberOfItems`, optional **`topicTid`** / **`dashboardTagTid`**, and `style` (e.g. padding)—see each block’s Zod schema and matching `*SidebarPanel` in `packages/editor-sample` (`RheumnowTopicSelect`, `RheumnowDashboardTagSelect`). Dropdowns use `/admin/terms` rows where **`vid`** is **`Topic`** or **`Dashboard tags`** (case-insensitive). **Dashboard tags** require **`field_is_active === 1`**. **Topics** use `vid === Topic` and treat empty or `1` as active; **`0`** / **`Off`** are excluded. **Advertisement 300×250, 728×90, and conference 300×250** blocks also support optional **`restrictToNoTopicAdvertisements`**: when true, the client parses the feed XML and keeps only items with an empty/absent `field_topics` (general ads), after the same topic URL segment is applied. **NewsPanelXml** supports optional **`isFeatured`**: when true, only feed items with **`field_is_featured === 1`** are included (`0`, empty, or absent values are excluded). Optional **`largeImageStyle`**: when true, renders articles (and tweets) in a stacked full-width image layout matching **Featured Story** (`width: 100%` hero image, play overlay for video types).
...
- Editor sample: Block inspector panels under `ConfigurationPanel/input-panels/` (XML URL inputs removed for the XML blocks above)
